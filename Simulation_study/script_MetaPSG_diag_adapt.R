...
# Running MetaPSG
    ...
    for (i in 1:n_alphas){
      ...
      Res_grid_alpha[[i]] <- f_selec_mod_metapsg_gv(sumstat, n_s, alpha=alpha_values[i], adaptive=TRUE, t_inc=2, npts=n_lambda_pts)
      # save MIC values
...
# SAVE
write.table(res_summary_table, file=paste0(outputdir,"simu_res_metapsg_diag_adapt.txt"), quote = FALSE)
write.table(Finaltime, file=paste0(outputdir,"time_metapsg_diag_adapt.txt"), quote = FALSE)
